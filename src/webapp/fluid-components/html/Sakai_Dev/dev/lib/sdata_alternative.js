...
sdata.widgets.WidgetLoader =  {
	...
	sethtmloverspecial : function (div,content,bigarray,widgetname){
		...
		for (var ii = 0; ii < bigarray[widgetname].length; ii++){
			document.getElementById(bigarray[widgetname][ii].uid).innerHTML=content;
		}
			...
			sdata.Ajax.request({
				...
				onSuccess : function(data) {
					...
					if (mejson.items.userLocale) {
						sdata.Ajax.request({
							url: "/dev/bundle/" + mejson.items.userLocale.language + "_" + mejson.items.userLocale.country + ".json",
							httpMethod: "GET",
							onSuccess: function(data){
