...
/*global jQuery*/
/*global fluid_1_3:true, debugMode:true, document*/
...
(function ($, fluid) {
    ...
    renderer.isBoundPrimitive = function (value) {
        return fluid.isPrimitive(value) ||
    ...
    function fixupValue(uibound, model, resolverGetConfig) {
        if (uibound.value === undefined && uibound.valuebinding !== undefined) {
            if (!model) {
                ...
                uibound.valuebinding +
                " since no model was supplied to rendering");
            }
        ...
        function resolveRecurse(basecontainer, parentlump) {
            ...
            if (parentlump.downmap) {
                for (var id in parentlump.downmap) {
                    if (parentlump.downmap.hasOwnProperty(id)) {
                        //if (id.indexOf(":") === -1) {
                        ...
                        for (var j = 0; j < lumps.length; ++j) {
                            ...
                            if (lumpid !== undefined && lump.rsfID !== undefined) {
                                var resolved2 = fetchComponent(basecontainer, lump.rsfID);
                                if (resolved2 !== null) {
                                    var resolveID = resolved2.fullID;
                                    if (resolved2.componentType === "UISelect") {
                ...
                // honour these remaining types immediately
                    ...
                    if (type === "attrs") {
                        ...
                        if (type === "addClass" || type === "removeClass") {
                            ...
                            if (type === "identify") {
                                var id2 = adjustForID(attrcopy, torender, true, finalID);
                                renderOptions.idMap[decorator.key] = id2;
                            }
                            else 
                                if (type !== "null") {
                ...
                if (componentType === "UISelect") {
                    ...
                    if (ishtmlselect) {
                        ...
                        if (!names || !names.length) {
                            ...
                            torender.fullID +
                            " does not have optionnames set");
                        }
                        for (var i = 0; i < names.length; ++i) {
                            out += "<option value=\"";
                            var val = values[i];
                            if (val === null) {
                                val = fluid.NULL_STRING;
                            }
                            out += fluid.XMLEncode(val);
                            if (isSelectedValue(torender, val)) {
                    ...
                    if (componentType === "UILink") {
                        ...
                        if (torender.markup !== undefined) { // detect UIVerbatim
                            ...
                            if (child.children) { // it is a branch 
                                ...
                                if (targetlump) {
                                    if (debugMode) {
                                        ...
                                        fluid.debugLump(lump) +
                                        " to " +
                                        fluid.debugLump(targetlump));
                                    }
                                    ...
                                    if (debugMode) {
                                        renderDebugMessage("No matching template branch found for branch container with full ID " +
                                        child.fullID +
            ...
            if (debugMode) {
                ...
                for (var key = 0; key < children.length; ++key) {
                    ...
                    if (!rendered[child.fullID]) {
                        ...
                        " with full ID " +
                        child.fullID +
                        " could not be found within template " +
                        fluid.debugLump(baselump));
                    }
                }
            }
