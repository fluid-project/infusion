 ...
 */
/*global jQuery, fluid_1_3:true, XMLLump:true, processTagEnd */
...
(function ($, fluid) {
    ...
    fluid.parseTemplate = function (template, baseURL, scanStart, cutpoints_in, opts) {
                    ...
                    if (isMatch) {
                        ...
                        if (cutstat.length === cut.tree.length) {
                            if (togo !== undefined) {
                                ...
                                debugLump(headlump) +
                                " has already matched with rsf:id of " +
                                togo);
                            }
                            if (cut.id === undefined || cut.id === null) {
                                fluid.fail("Error in cutpoints list - entry at position " + j + " does not have an id set");
                            }
                            togo = cut.id;
                        }
                    }
        ...
        if (excess) {
            ...
            ": " +
            fluid.transform(tagstack.splice(1, excess), function (lump) {
    ...
    fluid.dumpAttributes = function (attrcopy) {
        ...
        for (var attrname in attrcopy) {
            if (attrcopy.hasOwnProperty(attrname)) {
