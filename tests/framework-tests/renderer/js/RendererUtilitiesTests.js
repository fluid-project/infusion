...
(function ($) {
    ...
    fluid.defaults("fluid.tests.fluid5592root", {
        ...
        components: {
            render: {
                type: "fluid.tests.fluid5592render",
                container: "{fluid5592root}.container"
            }
        }
    });

    fluid.defaults("fluid.tests.fluid5592render", {
        gradeNames: ["fluid.rendererRelayComponent", "autoInit"],
        model: {
        ...
        selectorsToIgnore: ["sub"],
        renderOnInit: true,
        components: {
            sub: {
                type: "fluid.rendererRelayComponent",
                container: "{fluid5592root}.dom.sub",
                options: {
                    model: {
                        submodelValue: "{fluid5592render}.model.renderValue"
                    },
                    modelListeners: {
                        submodelValue: {
    ...
    jqUnit.test("FLUID-5592: Error received using model relay to destroyed component", function () {
        var that = fluid.tests.fluid5592root(".FLUID-5592");
        jqUnit.assertValue("The initial model relay has set the target model value", 1, that.render.sub.model.submodelValue);

        that.applier.change("value", 2);
        jqUnit.assertValue("The change request on the source model has set the target model value", 2, that.render.sub.model.submodelValue);

        that.render.applier.change("renderValue", 3);
        jqUnit.assertValue("The change request on the source model has set the target model value", 3, that.render.sub.model.submodelValue);

        that.render.destroy();
        that.refreshView();

        that.applier.change("value", 4);
    });
