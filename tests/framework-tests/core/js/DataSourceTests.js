...
(function () {
    ...
    fluid.tests.queuedDataSource.record = function (record, request) {
        record.push(request.directModel);
    };

    fluid.tests.queuedDataSource.requestFn = function (directModel, callback) {
        // use a setTimeout to simulate an asynchronous request
        // setTimeout(callback, 100);
        callback(directModel);
    };

    fluid.tests.queuedDataSource.requestSetFn = function (directModel, model, callback) {
        // use a setTimeout to simulate an asynchronous request
        // setTimeout(callback, 100);
        callback(directModel, model);
    };

    fluid.tests.queuedDataSource.assertRequests = function (that, expectedRequests) {
        jqUnit.assertDeepEq("The requests should have been processed through the queue corectly.", expectedRequests, that.fireRecord);
        jqUnit.start();
    };
    ...
    jqUnit.asyncTest("Queued DataSource", function () {
        fluid.tests.queuedDataSource({
            listeners: {
                "onCreate": [{
                    listener: "{that}.get",
                    args: [{path: "get"}, fluid.identity]
                }, {
                    listener: "{that}.set",
                    args: [{path: "set"}, {key: "value"}, fluid.identity]
                }, {
                    listener: "{that}.delete",
                    args: [{path: "delete"}, fluid.identity]
                }, {
                //     listener: "fluid.tests.queuedDataSource.assertRequestsQueue",
                //     args: ["{that}", expectedReadQueues, expectedWriteQueues]
                // }, {
                    listener: "fluid.tests.queuedDataSource.assertRequests",
                    args: ["{that}", {}]
                }]
            }
        });
